...
class window_main(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):    
        ...
        self.geometry("1000x750+100+10") # Fensterbreite,hoehe, on secreen offset x, on screen offset y
        ...
        frame_measure = tk.LabelFrame(self, text="Measurment Settings",bg='grey') # width=100, height=300)
        frame_measure.grid(row=0, column=0,padx=frame_boarder, pady = frame_boarder,sticky="nsew")
        ...
        self.freq_start_lable = tk.Label(frame_measure,text="START frequency: ",background="green4")
        self.freq_start_lable.grid(row=1, column=0,padx=5, pady = 5)
        #simple_label("MHz",270,100)
        
        
        self.freq_start_input = tk.Entry(frame_measure,textvariable=freq_start,justify="right",fg="black", bg="white", width=10)
        self.freq_start_input.grid(row=1, column=1,sticky="ew",padx=5, pady = 5)
        ...
        # end frequency
        self.freq_end_lable = tk.Label(frame_measure,text="END frequency: ",background="red4")
        self.freq_end_lable.grid(row=2, column=0,padx=5, pady = 5)
        #simple_label("MHz",270,150)
        
        self.freq_end_input = tk.Entry(frame_measure,justify="right",fg="black", bg="white", width=10)
        ...
        # freq Steps
        self.freq_step_lable = tk.Label(frame_measure,text="frequency steps: ",background="dark goldenrod")
        ...
        self.freq_step_input = tk.Entry(frame_measure,justify="right",fg="black", bg="white", width=10)
        self.freq_step_input.grid(row=3, column=1,sticky="ew",padx=5, pady = 5)
        
        # average
        self.average_lable = tk.Label(frame_measure,text="average: ",background="dark goldenrod")
        self.average_lable.grid(row=4, column=0,padx=5, pady = 5)
        tk.Label(frame_measure,text="step").grid(row=4, column=2,padx=3) 
        
        self.average_input = tk.Entry(frame_measure,justify="right",fg="black", bg="white", width=10)
        self.average_input.grid(row=4, column=1,sticky="ew",padx=5, pady = 5)
        
        # Butten RUN measurement
        self.button_run = tk.Button(frame_measure, text = "RUN measurment", command = self.set_measur,foreground="green")
        self.button_run.grid(row=5, column=0,rowspan=3,padx=5, pady = 5,columnspan=2)
        
        
        ######----- Tune&Match Settings ------######
        frame_tm = tk.LabelFrame(self, text="Tune&Match Settings",bg='grey')
        frame_tm.grid(row=0, column=1,padx=frame_boarder, pady = frame_boarder,sticky="nsew")
        ...
        self.Tune_U_max_input = tk.Entry(frame_tm, fg="black", bg="white", width=10)
        ...
        self.Match_U_max_input = tk.Entry(frame_tm, fg="black", bg="white", width=10)
        ...
        self.V_step_input= tk.Entry(frame_tm,fg="black", bg="white", width=10)
        ...
        #Buttens
        self.send_TMfile = tk.Button(frame_tm, text = "Read TM-file", command = RUN,foreground="red4")
        self.send_TMfile.grid(row=4, column=0,padx=5, pady = 5)
        
        self.send_TMfile = tk.Button(frame_tm, text = "Send to Arduino", command = RUN,foreground="green")
        ...
        frame_seq.grid(row=0, column=2,padx=frame_boarder, pady = frame_boarder,sticky="nsew")
        ...
        experiment_path_input = tk.Entry(frame_seq, fg="black", bg="white", width=10)
        ...
        cycle_path_input = tk.Entry(frame_seq, fg="black", bg="white", width=10)
        cycle_path_input.grid(row=2, column=1,padx=5, pady = 5)
        
        
        puls_button = tk.Button(frame_seq, text="set Puls sequenz", command=win_seq_puls.windows_file)#windows_file) 
        puls_button.grid(row=3, column=0,columnspan=2,padx=2, pady = 2)
        
        spin_button = tk.Button(frame_seq, text="set Spin sequenz", command=win_seq_spin.windows_file)#windows_file) 
        spin_button.grid(row=4, column=0,columnspan=2,padx=2, pady = 2)
        
        own_button = tk.Button(frame_seq, text="set own sequenz", command=win_seq_own.windows_file)#windows_file) 
        own_button.grid(row=5, column=0,columnspan=2,padx=2, pady = 2)
        
        self.update()
        logger_win_main.info("win_main2 start class window_main lable and button")
        ...
        frame_plot.grid(row=1, column=1,sticky="nsew",columnspan=2,rowspan=2,padx=2, pady = 2)
        
        plot_text = tk.Label(frame_plot, text="Results of last run",foreground="green",background="white", font=("Arial Bold", 10))
        ...
        file_path_lable = tk.Label(frame_logger,text="debug logger: ",foreground="green",background="black")
        ...
        combobox = TTK.Combobox(frame_logger,width=35,textvariable=loglevel_console,values=['DEBUG', 'INFO', 'WARNING', 'ERROR', 'CRITICAL']) 
        #textvariable=level ,state='readonly'
        combobox.current(0)
        combobox.grid(row=1,column=0,sticky="ew")
        
        logtext_area = tk.scrolledtext.ScrolledText(frame_logger,width = 30, height = 8,font = ("Times New Roman",10))
        ...
        button_run = tk.Button(frame_Buttens, text = "RUN", command = load_values(),background="chartreuse4")
        button_run.grid(row=0,column=0,padx=2, pady = 2,sticky="ew")
        
        close_button = tk.Button(frame_Buttens, text = "Test",background="SkyBlue4", command =lambda:  print("hi"))
        close_button.grid(row=1,column=0,padx=2, pady = 2,sticky="ew")
        
        plot_button = tk.Button(frame_Buttens, text="ploter", command=win_plot.win_plot)#windows_file) 
        plot_button.grid(row=2, column=0,padx=2, pady = 2,sticky="ew")
        
        exit_button = tk.Button(frame_Buttens, text="Close",background="tomato4", command=self.destroy)
    ...
    def plot_live(self,s1="",t1="",s2="",t2=""):
        ...
        file = "/home/pi/Bach_arbeit/signals/live_scan_data.csv"
